...
| 3 | `/api/voice/transcribe` 503 due to empty `OPENAI_API_KEY` in prod env | 🟢 **CLOSED** (2026-05-08, deploy `moj15ftko` / commit `4d44692`) | `vercel env rm` + re-add via `printf '%s' "$key" \| vercel env add` to avoid the trailing-newline gotcha (same one that bit STRIPE_USE_MOCK earlier). Prod key is now 164 bytes ending in `...CbMA`, matching the local `.env.local` value. Verified: authenticated `POST /api/voice/transcribe` (no audio body) now returns `400 {"error":"audio file required"}` — i.e. it gets past the API-key check and reaches body validation. Was previously `503 {"error":"Speech-to-text not configured", "code":"SERVICE_NOT_CONFIGURED"}`. |
