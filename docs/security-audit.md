...
| 5.4 | Input validation (zod) | 20/253 routes + reusable helpers in `lib/validation/common.ts`. Initial reference impl on `/api/billing/stub-checkout` (commit `5dc8efe`); +7 in second pass — `/api/voice/intent`, `/api/bookmarks`, `/api/core-obligations/[id]`, `/api/costs/intake/[id]`, `/api/integrations/qbo/sync`, `/api/memberships/[id]`, `/api/serial-components/[id]`. **Phase 15.5 closed `/api/costs/[id]` PATCH** — added `parsePatchBody` helper to `lib/validation/common.ts` that returns the validated body PLUS the set of explicitly-sent keys. Lets PATCH routes distinguish omitted vs null vs value (the `'X' in body` semantics that previously blocked zod adoption). 8 vitest cases verify empty-body no-op, single-field update, explicit null, omitted-field passthrough, invalid-type 400, invalid-enum 400, amount rounding, role gate. Remaining ~233 routes still need per-route schemas; PATCH-style routes can now follow the same `parsePatchBody` pattern. | 🟡 MEDIUM (in progress — Phase 15.5 close) |
