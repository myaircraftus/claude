...
-- Migration-chain fix (2026-05-15 migration-repair sprint): a pre-Codex
-- `saved_parts` table already existed on the remote DB with an incompatible
-- legacy schema (12 columns, no `status`/`title`/`part_master_id`). That made
-- `CREATE TABLE IF NOT EXISTS` silently skip, and the unique index below
-- (which filters `WHERE status <> 'archived'`) failed with "column status
-- does not exist". The legacy table was verified EMPTY (0 rows) with zero
-- references in application code, so it is dropped here and recreated with
-- the canonical SOP schema.
DROP TABLE IF EXISTS saved_parts CASCADE;
