      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">{org?.name ?? 'My Organization'} · {(org?.plan ?? 'starter')} plan</p>
          </div>
                ...
                {queries.map(q => (
                  <Link key={q.id} href={`/history?query=${q.id}`}>
                    <div className="flex items-center gap-3 p-2 rounded-md hover:bg-accent transition-colors">
                      ...
                      <Badge variant={(confidenceColors[q.confidence] ?? 'secondary') as 'success' | 'warning' | 'danger' | 'secondary'} className="flex-shrink-0 text-xs">
