...
export default function AskPage() {
  ...
  function handleCitationSelect(citation: AnswerCitation) {
    ...
    // Fetch document file path for inline viewer
    const supabase = createBrowserSupabase()
    supabase.from('documents')
      ...
      .single()
      .then(({ data, error }) => {
        if (data?.file_path) setActiveDocPath(data.file_path)
        if (error) console.warn('[citation] could not fetch doc path:', error.message)
      })
  }
