...
CREATE TABLE aircraft_computed_status (
  ...
  -- Airframe time
  airframe_total_time         NUMERIC(8,1),
  airframe_time_source_date   DATE,

  -- Engine state
  engine_time_since_new       NUMERIC(8,1),
  engine_time_since_overhaul  NUMERIC(8,1),
  engine_last_overhaul_date   DATE,
  engine_last_overhaul_shop   TEXT,
  engine_tbo_hours            NUMERIC(8,1),
  engine_hours_to_tbo         NUMERIC(8,1),
...
CREATE UNIQUE INDEX idx_computed_status_aircraft ON aircraft_computed_status(aircraft_id);
