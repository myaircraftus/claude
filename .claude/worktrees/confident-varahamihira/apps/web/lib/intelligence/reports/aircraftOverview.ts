...
export async function generateAircraftOverviewReport(
  ...
  const narrativePrompt = `You are writing an Aircraft Overview Report for ${aircraft?.make} ${aircraft?.model} (${aircraft?.tail_number}).
...
Recent maintenance: ${events?.slice(0, 5).map((e: any) => `${e.entry_date}: ${e.event_type}`).join('; ')}.
Findings: ${findings?.length ?? 0} open issues (${findings?.filter((f: any) => f.severity === 'critical').length ?? 0} critical).
...
Write for an aircraft owner or buyer — not a mechanic.`
  ...
  const reportData = {
    ...
    recentMaintenance: events?.slice(0, 10).map((e: any) => ({
      date: e.entry_date,
      type: e.event_type,
