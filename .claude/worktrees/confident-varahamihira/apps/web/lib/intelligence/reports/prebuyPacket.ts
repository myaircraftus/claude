...
export async function generatePrebuyPacket(
  ...
  const [
    ...
    supabase.from('documents').select('id, doc_type, title').eq('aircraft_id', aircraftId),
  ])

  const majorRepairs =
    events?.filter((e: any) => ['major_repair', 'major_alteration'].includes(e.event_type)) ?? []
  const engineOverhauls =
    events?.filter((e: any) =>
  ...
  const annuals = events?.filter((e: any) => e.event_type === 'annual_inspection') ?? []

  const damageIndicators = majorRepairs.filter((e: any) => {
    const desc = (e.work_description ?? '').toLowerCase()
    return (
      desc.includes('damage') ||
      desc.includes('bent') ||
      desc.includes('fire') ||
      desc.includes('ground loop') ||
      desc.includes('prop strike') ||
      desc.includes('gear collapse')
    )
  })

  const audienceContext: Record<PacketAudience, string> = {
    prebuy_packet:
      'You are preparing a prebuy inspection summary for an aircraft buyer. Focus on airworthiness risks, value-affecting issues, and what a buyer should know.',
    lender_packet:
      'You are preparing a collateral assessment for an aircraft lender. Focus on document completeness, compliance status, and factors affecting the aircraft as loan security.',
    insurer_packet:
      'You are preparing an underwriting summary for an aviation insurance company. Focus on maintenance history, accident/incident indicators, compliance status, and risk factors.',
  }

  const executiveSummaryPrompt = `${audienceContext[audience]}
...
Annual Status: ${status?.annual_is_current ? 'Current' : 'OVERDUE'}, last ${status?.last_annual_date ?? 'Unknown'}
AD Status: ${status?.ads_complied ?? 0} complied, ${status?.ads_open ?? 0} open, ${status?.ads_unknown ?? 0} unknown
Major Repairs: ${majorRepairs.length}
Damage Indicators: ${damageIndicators.length}
Open Findings: ${findings?.length ?? 0} (${findings?.filter((f: any) => f.severity === 'critical').length ?? 0} critical)
...
Flag the most important issues in the first paragraph.`
  ...
  const reportData = {
    reportType:
      audience === 'prebuy_packet'
        ? 'Prebuy Summary Packet'
        : audience === 'lender_packet'
        ? 'Lender Collateral Summary'
    ...
    majorEvents: {
      ...
      majorRepairs: majorRepairs.map((e: any) => ({
        date: e.entry_date,
        summary: e.work_summary,
        description: e.work_description,
      })),
      damageIndicators: damageIndicators.map((e: any) => ({
        date: e.entry_date,
        summary: e.work_summary,
        description: e.work_description,
      })),
    ...
    adSummary: {
      ...
      openAds: adRecords?.filter((a: any) => a.compliance_status === 'open').map((a: any) => a.ad_number),
    },
    documentChecklist: {
      hasRegistration: status?.has_registration,
      hasAirworthinessCert: status?.has_airworthiness_cert,
      hasWeightBalance: status?.has_weight_balance,
      hasForm337s: documents?.some((d: any) => d.doc_type === 'form_337'),
      hasEngineLogbooks: documents?.some((d: any) => d.doc_type === 'engine_log'),
      hasAirframeLogbooks: documents?.some((d: any) => d.doc_type === 'airframe_log'),
      hasPropLogbooks: documents?.some((d: any) => d.doc_type === 'prop_log'),
    },
  }
