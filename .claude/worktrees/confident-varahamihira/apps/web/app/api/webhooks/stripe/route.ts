  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        const { aircraft_id, report_type, user_id, organization_id } = session.metadata ?? {}

        if (aircraft_id && report_type && user_id) {
          const REPORT_TYPES = ['prebuy_packet', 'lender_packet', 'insurer_packet']
          if (REPORT_TYPES.includes(report_type)) {
            ...
            if (job) {
              generateReport(job.id).catch(err =>
                console.error(`[stripe-webhook] Report generation failed for job ${job.id}:`, err)
              )
            }
          }
        }
