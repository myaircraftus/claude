...
export default async function SettingsPage({
  ...
  // Fetch members — use explicit FK hint to avoid ambiguity (user_id vs invited_by both ref user_profiles)
  const { data: members } = await supabase
    .from('organization_memberships')
    .select('id, role, invited_at, accepted_at, user_profiles!user_id(id, email, full_name, avatar_url)')
    .eq('organization_id', orgId)
    .order('invited_at', { nullsFirst: false })
