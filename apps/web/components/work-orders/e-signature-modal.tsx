 ...
 *
 * Ported from the Figma design system. Compliant with:
 *  • U.S. ESIGN Act (15 U.S.C. § 7001)
 *  • UETA (Uniform Electronic Transactions Act)
 *  • FAA 14 CFR Part 43.9 (Certificate of Return to Service)
 *
 * Modes: Draw (canvas/finger) | Type (cursive render via web fonts)
 * Audit captured: timestamp, IP, session token, consent record, document id.
 *
 * The result object is opaque from the modal's perspective — caller is
 * responsible for persisting the SignatureResult to wherever the signed
 * artifact lives (logbook_entries, invoices, work_orders, estimates).
 */

import React, { useState, useRef, useEffect } from 'react'
import {
  Shield, X, CheckCircle, RotateCcw, PenLine, Type,
  Lock, Clock, Globe, FileCheck, AlertCircle, BadgeCheck, ChevronDown,
} from 'lucide-react'
...
export interface ESignatureModalProps {
  ...
  signerTitle: string
  signerCert: string
  /** Extra context shown in the signer-identity panel */
...
const SIGNATURE_FONTS = [
  ...
  { label: 'Formal',  family: "'Great Vibes', cursive" },
  { label: 'Classic', family: "'Georgia', serif" },
]

const fakeIp = () => `${198}.${51}.${100}.${Math.floor(Math.random() * 254) + 1}`
const genToken = () => {
  const a = () => Math.random().toString(36).substring(2, 6).toUpperCase()
  return `${a()}-${a()}-${a()}-${a()}`
}

const DOC_TYPE_LABEL: Record<ESignatureModalProps['documentType'], string> = {
  invoice: 'Invoice',
...
export function ESignatureModal({
  ...
  // Stable session metadata captured at mount
  ...
  // Global mouse listeners — drag CAN leave the canvas mid-stroke
  useEffect(() => {
        ...
        <div className="bg-[#0A1628] px-6 py-4 shrink-0 flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-white/50 text-[11px] mt-0.5">{DOC_TYPE_LABEL[documentType]} · {documentTitle}</div>
            </div>
          </div>
          {step === 'sign' && (
              ...
              <div className="px-6 pt-5 pb-4 space-y-4">
                ...
                <div className="bg-[#F4F6FA] rounded-xl border border-[#E2E8F0] p-4 space-y-2">
                  ...
                  {[
                    ...
                    <div key={row.label} className="flex items-baseline gap-2 text-[12px]">
                      ...
                      <span className="text-foreground" style={{ fontWeight: row.label === 'Full Name' || row.label === 'Certificate' ? 700 : 500 }}>
                        {row.value}
                      </span>
                    </div>
                  ))}
                ...
                <AnimatePresence mode="wait">
                  {tab === 'draw' ? (
                      ...
                      <div className="relative border-2 border-dashed border-slate-200 rounded-xl overflow-hidden bg-white" style={{ height: 168 }}>
                        ...
                        {!hasDrawn && (
                          <div className="absolute inset-0 flex items-end justify-center pb-14 pointer-events-none">
                            <span
                              className="text-[18px] text-muted-foreground/25"
                    ...
                    <motion.div key="type" initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.12 }} className="space-y-3">
                      <div className="relative">
                        <button
                        ...
                        <AnimatePresence>
                          {fontMenuOpen && (
                            <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute top-full left-0 mt-1 bg-white border border-border rounded-xl shadow-xl z-20 min-w-[200px] overflow-hidden">
                              {SIGNATURE_FONTS.map((f, idx) => (
                                <button
                                  key={f.label}
                                  onClick={() => { setFontIdx(idx); setFontMenuOpen(false) }}
                      ...
                      <div className="border-2 border-dashed border-slate-200 rounded-xl px-6 py-5 bg-white min-h-[110px] relative">
                        ...
                        <input
                          ...
                          style={{ fontFamily: selectedFont.family, fontSize: '32px', color: '#0A1628', lineHeight: 1.3 }}
                          autoFocus
                          maxLength={60}
                        />
                ...
                <label className="flex items-start gap-3 cursor-pointer bg-slate-50 border border-slate-200 rounded-xl p-4">
                  ...
                  <span className="text-[11px] text-muted-foreground leading-relaxed">
                    ...
                    <strong>{DOC_TYPE_LABEL[documentType]}</strong> is true and accurate to the best of my knowledge. I understand this
...
/**
 * Renders the signed proof block beneath a document. Pure display —
 * pulls the SignatureResult from props, doesn't read storage.
 */
  ...
  return (
    <div className="border border-emerald-200 bg-emerald-50/50 rounded-xl overflow-hidden">
        ...
        <div>
          ...
          <div className="mt-1.5 flex items-center gap-1.5">
            ...
            <span className="text-[11px] text-emerald-700" style={{ fontWeight: 600 }}>
              {sig.signerName} · {sig.signerTitle}
            </span>
          </div>
        </div>
          ...
          {[
            ...
            <div key={row.label} className="flex gap-2 text-[11px]">
              ...
              <span
                ...
                style={{ fontWeight: 500, fontFamily: row.label === 'Session ID' || row.label === 'Document' ? 'monospace' : undefined }}
              >
                {row.value}
              </span>
            </div>
      ...
      <div className="border-t border-emerald-200 px-4 py-2 bg-emerald-50 text-[10px] text-emerald-700 leading-relaxed">
        ...
        The signer&rsquo;s IP address, timestamp, and consent record are permanently associated with this document. This electronic signature is legally equivalent to a wet-ink signature.
      </div>
    </div>
  )
}
