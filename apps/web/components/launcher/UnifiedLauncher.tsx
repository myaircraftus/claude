...
export function UnifiedLauncher({
  ...
  useEffect(() => {
    ...
    if (saved === 'help' || saved === 'ask' || saved === 'inbox' || saved === 'messages') {
      setTab(saved)
    }
  }, [])
  useEffect(() => {
      ...
      {open && (
        <div className="fixed inset-0 z-40 flex items-end justify-end p-4 pointer-events-none">
          ...
          <div className="flex border-b border-slate-200 bg-white">
            ...
            <TabButton active={tab === 'ask'} onClick={() => setTab('ask')} icon={Sparkles} label="Ask" />
          ...
          <div className="flex-1 min-h-0 flex flex-col">
            ...
            {tab === 'inbox' && (
              <div className="flex-1 overflow-y-auto px-4 py-4 text-sm text-slate-700">
                <Inbox className="w-8 h-8 text-violet-500 mb-2" />
                <p className="font-semibold text-slate-900 mb-1">Unified inbox</p>
                <p className="text-xs text-slate-600 leading-relaxed mb-3">
                  Email + SMS to your myaircraft address, AI-categorised
                  (receipts, estimates, invoices, reminders). Reply, approve,
                  or file from one place — no jumping to another app.
                </p>
                <button
                  type="button"
                  onClick={() => {
