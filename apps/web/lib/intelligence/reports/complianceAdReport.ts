...
export async function generateComplianceAdReport(
  ...
  const reportData = {
    reportType: 'Compliance / AD Report',
    ...
    adSummary: {
      ...
      complied: compliedAds.length,
      open: openAds.length,
      unknown: unknownAds.length,
      openAdNumbers: openAds.map(a => a.ad_number).filter(Boolean),
      unknownAdNumbers: unknownAds.map(a => a.ad_number).filter(Boolean),
    },
  ...
  return renderReportToPDF(reportData, 'compliance_ad_report')
}
