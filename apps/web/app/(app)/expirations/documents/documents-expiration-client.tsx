...
/**
 * Aircraft & Shop Documents — expirations surface.
 *
 * Modeled on the /expirations/tools page: header + Add button, status filter
 * tabs (All / Expired / Expiring Soon / Valid), a table with colored status
 * badges, and a row-click side panel with detail, an inline edit form, and a
 * delete action. Status is derived from expiration_date via lib/expirations/status.
 */

import { useMemo, useState, type ReactNode } from 'react'
import { useRouter } from 'next/navigation'
import { toast } from 'sonner'
import { X, FileText, Plus, Loader2, Upload, Trash2, Save } from 'lucide-react'
...
const EMPTY_FORM = {
  document_name: '', aircraft_id: '', document_type: DOCUMENT_TYPES[0],
  document_number: '', issuing_authority: '', issue_date: '', expiration_date: '',
  notes: '',
}

export function DocumentsExpirationClient({
  documents, aircraft,
}: {
  ...
  const tailById = useMemo(() => {
    ...
    for (const a of aircraft) m[a.id] = a.tail_number
    return m
  }, [aircraft])
  ...
  async function createDocument(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/document-expirations', {
        ...
        body: JSON.stringify({
          scope: form.aircraft_id ? 'aircraft' : 'shop',
      ...
      <div className="px-6 py-4 border-b border-border bg-white shrink-0 flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-[20px] tracking-tight text-foreground" style={{ fontWeight: 700 }}>
            Aircraft &amp; Shop Documents
          </h1>
          <p className="text-[12px] text-muted-foreground mt-0.5">
            Expiration dates across certificates, registrations, and shop paperwork.
          </p>
        </div>
                ...
                {filtered.map(({ doc, status }) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-[12px] text-muted-foreground tabular-nums">
                      {doc.aircraft_id ? (tailById[doc.aircraft_id] ?? '—') : '—'}
                    </td>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-3 p-6">
      ...
      <p className="text-xs text-muted-foreground text-center max-w-sm">
        Add aircraft certificates and shop documents to track their expiration dates.
      </p>
...
function DocumentPanel({
  ...
  const set = (k: keyof typeof edit, v: string) => setEdit({ ...edit, [k]: v })
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/document-expirations/${doc.id}`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          scope: edit.aircraft_id ? 'aircraft' : 'shop',
          aircraft_id: edit.aircraft_id || null,
          document_name: edit.document_name.trim(),
          document_type: edit.document_type || null,
          document_number: edit.document_number.trim() || null,
          issuing_authority: edit.issuing_authority.trim() || null,
      ...
      const j = await res.json().catch(() => ({}))
      if (!res.ok) {
        toast.error(j?.error ?? 'Could not save document')
        return
      }
      ...
      <div className="fixed right-0 top-0 z-50 h-full w-full max-w-[460px] bg-background border-l border-border shadow-xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          <PanelSection title="Document Details">
            <DetailRow label="Aircraft" value={doc.aircraft_id ? (aircraft.find((a) => a.id === doc.aircraft_id)?.tail_number ?? null) : null} />
          ...
          <PanelSection title="Edit Document">
            <form onSubmit={save} className="space-y-3">
              <Field label="Document Name *"><Text value={edit.document_name} onChange={(v) => set('document_name', v)} /></Field>
              <Field label="Aircraft">
                <select
                  value={edit.aircraft_id}
                ...
                >
                  ...
                  {aircraft.map((a) => (
                    <option key={a.id} value={a.id}>
                      {a.tail_number}{[a.make, a.model].filter(Boolean).length ? ` — ${[a.make, a.model].filter(Boolean).join(' ')}` : ''}
                    </option>
                  ))}
                </select>
              </Field>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Field label="Document #"><Text value={edit.document_number} onChange={(v) => set('document_number', v)} /></Field>
                <Field label="Issuing Authority"><Text value={edit.issuing_authority} onChange={(v) => set('issuing_authority', v)} /></Field>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <Field label="Issue Date"><Text type="date" value={edit.issue_date} onChange={(v) => set('issue_date', v)} /></Field>
                <Field label="Expiration Date"><Text type="date" value={edit.expiration_date} onChange={(v) => set('expiration_date', v)} /></Field>
              </div>
              <Field label="Notes"><Text value={edit.notes} onChange={(v) => set('notes', v)} /></Field>
