// Aircraft Intelligence — Full History Package.
...
import { HistoryClient, type HistoryReport } from './history-client'

export const dynamic = 'force-dynamic'
export const metadata = { title: 'Full History Package' }

export default async function FullHistoryPage({ params }: { params: { id: string } }) {
  ...
  let initialReport: HistoryReport | null = null
  try {
    const cached = await readIntelligenceCache(createServiceSupabase(), aircraftId, 'history')
    if (cached) {
      initialReport = { ...(cached.result_json as object), cached: true } as HistoryReport
    }
  } catch {
    initialReport = null
  }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      />
      <IntelligenceModuleNav aircraftId={aircraftId} active="history" />
      <main className="flex-1 overflow-y-auto">
        <HistoryClient aircraftId={aircraftId} initialReport={initialReport} />
      </main>
    </div>
  )
}
