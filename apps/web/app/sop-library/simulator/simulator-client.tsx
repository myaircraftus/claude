...
import { ArrowLeft, Loader2, Send, Sparkles, CheckCircle2, RefreshCcw, Printer } from 'lucide-react'
...
export function SimulatorClient() {
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
        ...
        <header className="mb-6 pb-5 border-b border-slate-800">
          <div className="flex items-center gap-2 text-violet-400 mb-2">
            ...
            <span className="text-[10px] uppercase tracking-[0.2em] font-semibold">AI Simulator</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {scenarios.map((s) => (
              <button
                ...
                className="group text-left rounded-lg border border-slate-800 bg-[#0f172a] hover:bg-[#131c2e] hover:border-violet-500/40 transition-colors p-5"
              >
                <div className="flex items-start justify-between mb-3">
                  <span className="text-[10px] uppercase tracking-[0.15em] text-violet-400 font-semibold">
                    {s.persona}
                  </span>
                ...
                <h2 className="text-base font-semibold text-white mb-2 leading-snug">{s.title}</h2>
  ...
  // ── Chat view ──────────────────────────────────────────────────
  return (
    <div className="max-w-5xl mx-auto px-6 py-6 flex flex-col h-[calc(100vh-2rem)]">
      {/* Header bar */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="text-xs text-slate-300">
            <span className="text-violet-400 font-semibold">{active.title}</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={restart}
            className="inline-flex items-center gap-1 text-[11px] text-slate-400 hover:text-white border border-slate-800 bg-[#0f172a] hover:bg-slate-800/40 rounded-md px-2.5 py-1 transition-colors"
          >
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-1.5">
          ...
          {scenarioComplete && (
            <div className="text-[10px] uppercase tracking-[0.15em] text-emerald-400 font-semibold flex items-center gap-1">
          ...
          {active.successCriteria.map((_, i) => (
            <div
              ...
              className={`flex-1 h-1.5 rounded-full transition-colors ${
                i < completedCriteria.length ? 'bg-emerald-500/70' : 'bg-slate-800'
              }`}
            />
          ))}
        </div>
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto rounded-lg border border-slate-800 bg-[#0a1020] p-4 space-y-4"
      >
        {messages.map((m, i) => (
      ...
      {!scenarioComplete && (
        <div className="mt-4 flex items-end gap-2">
          <textarea
            ...
            className="flex-1 resize-none bg-[#0f172a] border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/40 disabled:opacity-60"
          />
          <button
            type="button"
            onClick={sendTurn}
            disabled={!input.trim() || busy}
            className="inline-flex items-center gap-1.5 bg-violet-600 hover:bg-violet-500 disabled:bg-slate-700 disabled:text-slate-500 text-white text-sm font-medium rounded-lg px-4 py-2 transition-colors"
          >
  ...
  if (role === 'user') {
    return (
      <div className="flex justify-end">
        <div className="max-w-[80%] bg-violet-600/20 border border-violet-500/30 rounded-lg rounded-tr-sm px-3 py-2 text-sm text-slate-100">
          {content}
        </div>
      </div>
    )
  ...
  return (
    <div className="flex justify-start">
      <div className="max-w-[85%]">
        <div className="text-[10px] uppercase tracking-[0.15em] text-violet-400 font-semibold mb-1.5 flex items-center gap-1">
...
function CompletionCertificate({
  ...
  return (
    <div className="mt-4 rounded-lg border-2 border-emerald-500/40 bg-gradient-to-br from-emerald-500/5 to-slate-900 p-6 text-center print:bg-white print:text-black print:border-emerald-700">
      <div className="text-[10px] uppercase tracking-[0.2em] text-emerald-400 font-semibold mb-3 print:text-emerald-700">
        Completion Certificate
      </div>
      <CheckCircle2 className="w-10 h-10 text-emerald-400 mx-auto mb-2 print:text-emerald-700" />
      <div className="text-lg font-semibold text-white mb-1 print:text-black">{scenarioTitle}</div>
      <div className="text-xs text-slate-400 mb-4 print:text-slate-700">
      ...
      <div className="text-left max-w-md mx-auto">
        ...
        <ul className="space-y-1">
          {criteria.map((c, i) => (
            <li key={i} className="text-xs text-slate-300 flex items-start gap-2 print:text-black">
              <CheckCircle2 className="w-3 h-3 text-emerald-400 mt-0.5 shrink-0 print:text-emerald-700" />
              <span>{c}</span>
            </li>
          ))}
        </ul>
      ...
      <button
        ...
        className="mt-5 inline-flex items-center gap-1.5 text-xs text-emerald-200 hover:text-white bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/40 rounded-md px-3 py-1.5 transition-colors print:hidden"
      >
