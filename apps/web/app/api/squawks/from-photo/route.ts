...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`squawks-from-photo:${getClientIp(req.headers)}`, { limit: 10, windowSeconds: 60 })
  ...
  try {
    ...
    return NextResponse.json({ error: 'Invalid form data — send image as multipart/form-data with field name "image".' }, { status: 400 })
  }
